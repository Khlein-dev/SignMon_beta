...
const LESSON8_PASSED_KEY = "lesson8Passed";
const QUIZ8_FINISHED_KEY = "quiz8Finished";

const REVIEW_ROUTE = "/lessons/lesson8";
...
export default function Quiz8() {
    ...
    const markLessonPassed = useCallback(async () => {
        try {
            await AsyncStorage.multiSet([
                [LESSON8_PASSED_KEY, "true"],
                [QUIZ8_FINISHED_KEY, "true"],
            ]);
        } catch (error) {
            console.log("Failed to save lesson 8 pass state:", error);
        }
    }, []);
    ...
    const handleChoice = useCallback(
        ...
        [locked, currentQuestion, currentIndex, score, finishQuiz, playPop, playCorrect]
    );
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.title}>Quiz 8</Text>
            ...
            <Animated.View
                style={[
                    styles.quizCard,
                    {
                        transform: [{ scale: cardScale }, { rotate: cardRotation }],
                    },
                ]}
            >
                <Text style={styles.quizLabel}>Anong araw ang ipinapakita sa video?</Text>
                ...
                <Text style={styles.quizSubtext}>
                    Panoorin ang sign at piliin ang tamang araw.
                </Text>
            </Animated.View>
                    ...
                    >
                        {didWin ? (
                            <>
                                ...
                                <Text style={styles.modalSubText}>
                                    Passed na ang Lesson 8. Mahusay!
                                </Text>
...
function generateQuestions() {
    ...
    for (let i = 0; i < QUESTION_COUNT; i += 1) {
        const item = DAY_BANK[Math.floor(Math.random() * DAY_BANK.length)];
        const wrongChoices = shuffleArray(
            DAY_BANK.filter((c) => c.answer !== item.answer).map((c) => c.answer)
        ).slice(0, 3);
